function [ x ] = generateLaplacian(image, trans_est)
...
%   Detailed explanation goes here
    ...
    for i = (1+win_size):(dimI(2)-win_size)
        for j = (1+win_size):(dimI(1)-win_size)
            ...
            L_element = eye(win_pixels) - (1 + diff' * (window_covariance \ diff)) ./ win_pixels;
    ...
    x = (L + .00001 .* speye(size(L))) \ trans_est(:) .* .00001;
    x = reshape(x, size(trans_est));
